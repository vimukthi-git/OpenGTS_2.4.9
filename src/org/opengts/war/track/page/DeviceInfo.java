            ...
            HTML_CONTENT = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
                public void write(PrintWriter out) throws IOException {
                  ...
                  //String editURL    = DeviceInfo.this.encodePageURL(reqState);//,RequestProperties.TRACK_BASE_URI());
                    ...
                    /* command state */
                    Vector<Tuple.Quad<Integer,String,String,String>> cmdStateList = null;
                    for (int i = 0; i < 8; i++) {
                        ...
                        String v[]   = StringTools.split(val,','); // Title,False,True
                        String title = AccountRecord.GetSimpleLocalString((v.length > 0)? v[0] : "State", locale); 
                        String off   = AccountRecord.GetSimpleLocalString((v.length > 1)? v[1] : "0"    , locale);
                        String on    = AccountRecord.GetSimpleLocalString((v.length > 2)? v[2] : "1"    , locale);
                        //Print.logInfo("Found CommandState #"+i+": "+title+" => "+off+"/"+on);
                        if (cmdStateList == null) { cmdStateList = new Vector<Tuple.Quad<Integer,String,String,String>>(); }
                        cmdStateList.add(new Tuple.Quad<Integer,String,String,String>(new Integer(i),title,off,on));
                    }
                    ...
                    if (cmdStateList != null) {
                    for (Tuple.Quad<Integer,String,String,String> cs : cmdStateList) { // Index,Title,False,True
                    String cmdState_title = cs.b;
                    out.write("   <th class='"+CommonServlet.CSS_ADMIN_TABLE_HEADER_COL    +"' nowrap>"+FilterText(cmdState_title)+"</th>\n");
                    }
                    }
                    if (showAcks) {
                    ...
                    for (int d = 0; d < _deviceList.size(); d++) {
                        ...
                        try {
                            ...
                            if (dev != null) {
                                ...
                                if (cmdStateList != null) {
                                for (Tuple.Quad<Integer,String,String,String> cs : cmdStateList) { // Index,Title,False,True
                                int    cmdState_ndx = cs.a.intValue();
                                String cmdState_val = dev.getCommandStateMaskBit(cmdState_ndx)? cs.d : cs.c;
                                out.write("   <td class='"+CommonServlet.CSS_ADMIN_TABLE_BODY_COL    +"' nowrap>"+cmdState_val+"</td>\n");
                                }
                                }
                                if (showAcks) {
