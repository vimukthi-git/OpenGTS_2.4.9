    ...
    implements Comparable
{
  ...
  //public static final String  COMMAND_OUTPUT                  = "output";     // arg=gpioOutputState
    ...
    public static final String  KEYARG_QUOTE[]                  = { "q", "quote" };
    public static final String  KEYARG_GPLAT[]                  = { "gplat", "lat", "latitude"  };
    public static final String  KEYARG_GPLON[]                  = { "gplon", "lon", "longitude" };
    ...
    public class Command
    {
        ...
        public Command(
            String  name     , String desc, boolean enable,
            ...
            if (this.name.equals("GFMITextMessage")) {
                Print.logInfo("Name   : " + this.name);
        ...
        public boolean isEnabled(BasicPrivateLabel privLabel) {
            if (privLabel != null) {
                String dcsName = DCServerConfig.this.getName();
                String cmdName = this.getName();
                String rtKey   = // DCServer.DCSNAME.Command.CMDNAME.enabled
                    DCServerFactory.PROP_DCServer_ + dcsName + "." + DCServerFactory.PROP_Command_ + 
                    cmdName + "." + DCServerFactory.ATTR_enabled;
        ...
        private String _adjustKeyArg(String V, String keyArg) {
            if (!StringTools.isBlank(keyArg)) {
                ...
                if (ListTools.contains(KEYARG_GPLAT,ka)) {
                    // extract latitude from "latitude/longitude" [latitude=${gp:lat}]
                    int p = StringTools.indexOf(V,GP_SEP);
                    if (p < 0) {
                        // no separator
                        return V;
                    } else {
                        // LATITUDE,longitude ==> LATITUDE
                        return StringTools.stripChars(V.substring(0,p)," \t"); 
                    }
                } else
                if (ListTools.contains(KEYARG_GPLON,ka)) {
                    // extract longitude from "latitude/longitude" [longitude=${gp:lon}]
                    int p = StringTools.indexOf(V,GP_SEP);
                    if (p < 0) {
                        // no separator
                        return V;
                        ...
                        return StringTools.stripChars(V.substring(p+1)," \t"); 
                    }
                } else
                ...
                return StringTools.replaceKeys(cs, new StringTools.KeyValueMap() {
                    public String getKeyValue(String key, String keyArg, String dft) {
                        ...
                        if ((argNdx >= 0) && (argNdx < args.length)) { 
                            // ${text[:{quote|gplat|gplon}][=DFT]}
                            String V = (args[argNdx] != null)? args[argNdx] : dft;
                            return Command.this._adjustKeyArg(V,keyArg);
                        } else
                        if (key.equals(DEFAULT_ARG_NAME)) { 
                            // ${arg[:{quote|gplat|gplon}][=DFT]}
                            String V = ((args.length > 0) && (args[0] != null))? args[0] : dft; 
                            return Command.this._adjustKeyArg(V,keyArg);
                        } else {
                            // ${argX[:{quote|gplat|gplon}][=DFT]}
                            for (int i = 0; i < args.length; i++) {
                                if (key.equals(DEFAULT_ARG_NAME+i)) { // "arg0", "arg1", ...
                                    String V = (args[i] != null)? args[i] : dft; 
                                    return Command.this._adjustKeyArg(V,keyArg);
                                }
                            }
    ...
    **/
        ...
        throws SocketException, IOException
    {
        if (port > 0) {
            ...
            return new ServerSocketThread((DatagramSocket)null);
        }
    }
