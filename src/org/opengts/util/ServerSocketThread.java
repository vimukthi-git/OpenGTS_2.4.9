    ...
    extends Thread
{
    ...
    **/
    public interface SessionInfo
    {
        ...
        public long         getSessionStartTime();  // Epoch session start time
        public long         getSessionReceiveTime();// Epoch session last receive time
        ...
        implements SessionInfo
    {
        ...
        **/
        ...
        public boolean udpWrite(byte data[]) {
            ...
            if ((data != null) && (data.length > 0)) {
                synchronized (this.runLock) {
                    if ((this.client != null) && this.client.isUDP()) {
                        try {
        ...
        **/
        ...
        public void handleClientSession(ClientSocket clientSock) {
                            ...
                            try {
                                ...
                                if ((response != null) && (response.length > 0)) {
                                    ...
                                    if (clientSock.isInputStream()) {
                                        if (LogEnable) { Print.logInfo("Skipping InputStream response: 0x%s", StringTools.toHexString(response)); }
                                    } else {
                                        // ?
                                    }
                                } else {
            ...
            if (LogEnable) { Print.logInfo("End of " + clientSock.getSessionType() + " session ..."); }
            ...
            if (clientHandler != null) {
                try {
                    ...
                    if ((finalPacket != null) && (finalPacket.length > 0)) {
                        ...
                        if (clientSock.isInputStream()) {
                            if (LogEnable) { Print.logInfo("Skipping InputStream finalPacket: 0x" + StringTools.toHexString(finalPacket)); }
                        } else {
                            // 
                        }
                    }
