...
public class JSON
{
        ...
        throws JSONParsingException 
    {
        ...
        for (;ndx < len;) {
            ...
            if (ch == '}') {
                ...
                // invalid character
                //Print.logError("Syntax error at index " + ndx + "\n" + v.substring(0,ndx));
        ...
        for (;ndx < len;) {
            ...
            if (ch == 'n') { 
                ...
                if ((v.charAt(ndx  ) == 'u') && 
                    ...
                    throw new JSONParsingException("Invalid 'null'", ndx);
                }
                break valueParse;
            } else
            if (ch == '[') {
    ...
    **/
        ...
        extends Vector<JSON._Value>
    {
        ...
        **/
        public long getLongValueAt(int ndx, int dft) {
            if ((ndx >= 0) && (ndx < this.size())) {
                JSON._Value jv = this.get(ndx);
                return (jv != null)? jv.getLongValue(dft) : dft;
            } else {
    ...
    **/
    public static JSON._Array parse_Array(String v, AccumulatorLong index)
        throws JSONParsingException 
    {
        if (index == null) { index = new AccumulatorLong(0L); }
        int         ndx   = (int)index.get();
        int         len   = v.length();
